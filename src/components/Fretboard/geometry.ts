 ...
 */
export function fretDistanceFromNut(scaleLengthPx: number, k: number, sys: TuningSystem) {
  return scaleLengthPx - scaleLengthPx / Math.pow(2, k / sys.divisions);
}

/** Build an array of fret positions (x in px) for 1..fretCount. */
export function buildFrets(scaleLengthPx: number, fretCount: number, sys: TuningSystem) {
